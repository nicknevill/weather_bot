...
# ─── Настройки ───────────────────────────────────────────────
TELEGRAM_TOKEN = "8243476503:AAEoWnzuJngGKE8l7vciCaOuDpM5yJdyZh8"
WEATHER_API_KEY = "eeeec702d8452e08ae0176000af971d9"
IQAIR_API_KEY = "3aaccad9-774b-4e61-857d-9dfaaa4ca1cf"
...
def aqi_label(aqi: int) -> str:
    ...
    if aqi <= 100:   return "🙂 Хорошо"
    if aqi <= 150:   return "😐 Умеренно"
    if aqi <= 200:   return "😷 Плохо"
    if aqi <= 300:   return "🤢 Очень плохо"
    return "☠️ Опасно"

async def get_air_quality(lat: float, lon: float) -> str:
    url = (
        f"https://api.airvisual.com/v2/nearest_city"
        f"?lat={lat}&lon={lon}&key={IQAIR_API_KEY}"
    )
...
def main_menu():
    keyboard = [
        ...
        [KeyboardButton("🌬 Качество воздуха")]
    ]
...
async def cmd_air(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Получаю данные о воздухе...")
    text = await get_air_quality(lat, lon)
    if text:
        await update.message.reply_text(text, parse_mode="Markdown")
    else:
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "city":
        ...
        await update.message.reply_text("Используй кнопки меню внизу 👇", reply_markup=main_menu())
