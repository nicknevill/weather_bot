...
async def get_air_quality(lat: float, lon: float) -> str:
    ...
    return (
        f"🌬 *Качество воздуха*\n\n"
        f"Индекс AQI: *{label}*\n\n"
        f"• PM2.5: *{pm25:.1f}* мкг/м³\n"
        f"• PM10:  *{pm10:.1f}* мкг/м³\n"
        f"• NO₂:   *{no2:.1f}* мкг/м³\n"
        f"• O₃:    *{o3:.1f}* мкг/м³\n\n"
        f"_Данные: OpenWeatherMap_"
    )
...
async def cmd_air(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Если нет координат — получаем их через forecast API
    ...
    if text:
        ...
        await update.message.reply_text("❌ Не удалось получить данные о качестве воздуха.")
